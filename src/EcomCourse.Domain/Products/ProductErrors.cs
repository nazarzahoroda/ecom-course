...
public static class ProductErrors
{
    ...
    public static DomainError CategoryNotFound(Guid categoryId) =>
        new("Product.CategoryNotFound", $"Category {categoryId} was not found.");

    public static DomainError SKUAlreadyExists(string sku) =>
        new("Product.SKUAlreadyExists", $"Product with SKU {sku} already exists.");

    public static DomainError NotFound(Guid id) =>
        new("Product.NotFound", $"Product {id} was not found.");
}
